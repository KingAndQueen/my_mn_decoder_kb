...
def embedding_rnn_decoder(decoder_inputs, initial_state, category, cell, num_symbols, num_category,
    ...
    Args:
      ...
      category: qichuan import category information
    Returns:
      A tuple of the form (outputs, state), where:
        outputs: A list of the same length as decoder_inputs of 2D Tensors with
          shape [batch_size x cell.output_size] containing the generated outputs.
    ...
    """
    pdb.set_trace()
    category = category[:len(decoder_inputs)]
    if len(decoder_inputs) != len(category):
        print(len(decoder_inputs), len(category))
        raise ValueError("category lenght is not equal to deocder_inputs")
    ...
    with variable_scope.variable_scope(scope or "embedding_rnn_decoder"):
        with ops.device("/cpu:0"):
            embedding = variable_scope.get_variable("embedding",
                                                    [num_symbols, cell.input_size / 2])
            embedding_category = variable_scope.get_variable("embedding_category", [num_category, cell.input_size / 2])
        ...
        emb_category = [embedding_ops.embedding_lookup(embedding_category, i) for i in category]
        embedding_them = []
        #    pdb.set_trace()
        for i, _ in enumerate(emb_inp):
            embedding_them.append(tf.concat(1, [emb_inp[i], emb_category[i]]))
        #    pdb.set_trace()
        return rnn_decoder(embedding_them, initial_state, cell,
                           loop_function=loop_function)


def embedding_rnn_seq2seq(encoder_inputs, decoder_inputs, category, cell,
                          num_encoder_symbols, num_decoder_symbols, num_category,
                          output_projection=None, feed_previous=False,
    ...
    with variable_scope.variable_scope(scope or "embedding_rnn_seq2seq"):
        ...
        if isinstance(feed_previous, bool):
            return embedding_rnn_decoder(
                decoder_inputs, encoder_state, category, cell, num_decoder_symbols, num_category,
...
def my_embedding_rnn_seq2seq(encoder_inputs, decoder_inputs, cell,
                             num_encoder_symbols, num_decoder_symbols,
                             output_projection=None, feed_previous=False,
                             dtype=dtypes.float32, scope=None):
    """Embedding RNN sequence-to-sequence model.
    qichuan's modified version: use the same embedding in both encode and decode
    Args:
      encoder_inputs: A list of 1D int32 Tensors of shape [batch_size].
      decoder_inputs: A list of 1D int32 Tensors of shape [batch_size].
      cell: rnn_cell.RNNCell defining the cell function and size.
    ...
    with variable_scope.variable_scope(scope or "embedding_rnn_seq2seq"):
        # embedding
        unique_embedding = variable_scope.get_variable('embedding', [num_encoder_symbols, cell.output_size])
        emb_de_inp = [embedding_ops.embedding_lookup(unique_embedding, i) for i in decoder_inputs]
        emb_en_inp = [embedding_ops.embedding_lookup(unique_embedding, i) for i in encoder_inputs]
        # to detect error
        if output_projection is not None:
            proj_weights = ops.convert_to_tensor(
                output_projection[0], dtype=dtypes.float32)
            ...
            #   pdb.set_trace()
        # Encoder.
        _, encoder_state = rnn.rnn(cell, emb_en_inp, dtype=dtype)
        #    print(encoder_state)
        # Decoder.
        # temp_var=tf.Variable(encoder_state,'encoder_state')
        if output_projection is None:
            cell = rnn_cell.OutputProjectionWrapper(cell, num_decoder_symbols)
        ...
        def extract_argmax_and_embed(prev, _):
            ...
            prev_symbol = array_ops.stop_gradient(math_ops.argmax(prev, 1))
            return embedding_ops.embedding_lookup(unique_embedding, prev_symbol)

        loop_function = extract_argmax_and_embed if feed_previous else None
        if isinstance(feed_previous, bool):

            return rnn_decoder(emb_de_inp, encoder_state, cell, loop_function=loop_function)
        else:
            print('feed_previous is a Tensor')
            pdb.set_trace()
...
def sequence_loss_by_example(logits, targets, weights,
    ...
    with tf.name_scope(name, "sequence_loss_by_example"):
        ...
        # the weights should be a list of tensors to calculate the unzero word number _by qichuan
        new_weights = []
        for temp_weight in weights:
            new_weights.append(tf.convert_to_tensor(temp_weight, dtype=tf.float32))
            # pdb.set_trace()
        if average_across_timesteps:
            total_size = math_ops.add_n(new_weights)
...
def sequence_loss(logits, targets, weights,
    ...
    with tf.name_scope(name, "sequence_loss"):
        #  pdb.set_trace()
        loss_by_example = sequence_loss_by_example(
            logits, targets, weights,
            average_across_timesteps=average_across_timesteps,
            softmax_loss_function=softmax_loss_function)
        cost = math_ops.reduce_sum(loss_by_example)
